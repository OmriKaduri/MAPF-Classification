...
def train_all_models(df, exp_name, outer_splits, with_plots=True, maptype='', problem_type='', load=True):
    ...
    # nn_clf = NNClfModel(runtime_cols, max_runtime, offline_features_only, success_cols, maptype + problem_type)

    xgb_reg = XGBRegModel(runtime_cols, max_runtime, offline_features_only, success_cols, maptype + problem_type)
    ...
    #
    xgb_clf = XGBClfModel(runtime_cols, max_runtime, offline_features_only, success_cols, maptype + problem_type)
    ...
    xgb_coverage = CoverageClassifier(runtime_cols, max_runtime, offline_features_only, success_cols,
    ...
    for index, (tr_ind, test_ind) in enumerate(splits):
        ...
        if with_models:
            ...
            xgb_cost_sensitive.train_cv(X_train_offline,
                                        ...
                                        exp_type=exp_name)
            cost_preds = xgb_cost_sensitive.predict(X_test_offline, y_test)
    ...
    if with_models:
        results = [baselines.print_results(exp_dir=exp_name + '/'),
                   xgb_reg.print_results(exp_dir=exp_name + '/', with_header=False),
                   xgb_clf.print_results(exp_dir=exp_name + '/', with_header=False),
                   xgb_coverage.print_results(exp_dir=exp_name + '/', with_header=False),
                   xgb_cost_sensitive.print_results(exp_dir=exp_name + '/', with_header=False),
